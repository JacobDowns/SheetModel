from dolfin import *
from dolfin_adjoint import *
from sheet_model import *
from constants import *
from dolfin import MPI, mpi_comm_world
from numpy import *

""" This simulation starts from the sliding steady state generated by
sim_sliding_law_time.py. In this simulation sliding changes according to 
a sliding law and melt is varied over time.""" 

# Model input directory
in_dir = "inputs_sliding_law/"
# Output directory
out_dir = "out_low_bump/"
# Checkpoint directory
check_dir = out_dir + "checkpoint/"
# Process number
MPI_rank = MPI.rank(mpi_comm_world())

# Load mesh and create function spaces
mesh = Mesh(in_dir + "mesh_60_20.xml")
V_cg = FunctionSpace(mesh, "CG", 1)


### Model inputs

# Initial sheet height
h_init = Function(V_cg)
File(in_dir + "h_sliding_steady.xml") >> h_init

# Load the boundary facet function
boundaries = FacetFunction('size_t', mesh)
File(in_dir + "boundaries.xml") >> boundaries

# Load potential at 0 pressure
phi_m = Function(V_cg)
File(in_dir + "phi_m.xml") >> phi_m

# Driving stress
tau_b = Function(V_cg)
File(in_dir + "tau_b.xml") >> tau_b
  
# Load initial melt function
m = Function(V_cg) 
File(in_dir + "m_point2.xml") >> m
m1 = Function(V_cg)
m1.assign(m)

# Compute the total melt in a day
spd = pcs['spd']
mpd = assemble(m * dx) * spd

# Function that scales melt over time
def m_scale(t):
  return (1./3.) + (2./3.) * (cos((2.0 * pi / spd) * t) + 1.0)

# Initial sliding velocity
u_b = Function(V_cg)
File(in_dir + "u_b_steady.xml") >> u_b

# Enforce 0 pressure bc at margin
bc = DirichletBC(V_cg, phi_m, boundaries, 1)

# Use a slightly lower conductivity than the default

pcs['k'] = 5e-3
# Bump height
pcs['h_r'] = 0.1

prm = NonlinearVariationalSolver.default_parameters()
prm['newton_solver']['relaxation_parameter'] = 1.0
prm['newton_solver']['relative_tolerance'] = 1e-5
prm['newton_solver']['absolute_tolerance'] = 1e-5
prm['newton_solver']['error_on_nonconvergence'] = False
prm['newton_solver']['maximum_iterations'] = 25

model_inputs = {}
model_inputs['mesh'] = mesh
model_inputs['h_init'] = h_init
model_inputs['m'] = m
model_inputs['u_b'] = u_b
# Start at a time when the scale is 1
model_inputs['t_0'] = 0.25 * spd
model_inputs['d_bcs'] = [bc]
model_inputs['out_dir'] = out_dir
model_inputs['newton_params'] = prm
model_inputs['constants'] = pcs
model_inputs['opt_params'] = {'tol' : 0.5e-7, 'scale' : 15.0}

# Create the sheet model
model = SheetModel(model_inputs, in_dir)


### Run the simulation

# Seconds per minute
spm = 60.0
# Seconds per day
spd = pcs['spd']
# End time
T = 4.0 * spd
# Time step
dt = 10.0 * spm
# Iteration count
i = 0
# Somewhat arbitrary constant for sliding law
C = Constant(0.33e-8 / spy)
# Regularization param
u_b_reg = Constant(100.0)

while model.t < T:
  if MPI_rank == 0: 
    current_time = model.t / spd
    print "Current Time: " + str(current_time)
  
  model.step(dt)
  
  if i % 3 == 0:
    model.write_pvds(['h', 'pfo', 'u_b', 'm'])
    
  if i % 6 == 0:
    model.write_xmls()
  
  if MPI_rank == 0: 
    print
    
  # Update the melt
  model.update_m(project(Constant(m_scale(model.t)) * m1))
  
  i += 1